...
async fn work(correct_password: &str, sink: Arc<Mutex<SplitSink<rocket_ws::stream::DuplexStream, rocket_ws::Message>>>, stream: &mut SplitStream<rocket_ws::stream::DuplexStream>, #[cfg_attr(not(windows), allow(unused))] unhide_reboot: &mut bool, #[cfg_attr(not(windows), allow(unused))] unhide_sleep: &mut bool) -> Result<(), Error> {
    ...
    let min_disk_mount_points = min_disk_mount_points.into_iter().map(PathBuf::from).collect_vec();
    let mut work = pin!(ootrstats::worker::work(worker_tx, supervisor_rx, PathBuf::from(base_rom_path.clone()), 0, wsl_distro, rando_rev, setup, output_mode, min_disk, min_disk_percent, &min_disk_mount_points, &priority_users, race));
    loop {
