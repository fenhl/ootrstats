...
* `--draft`: Simulates a settings draft from the given file. See [`assets/draft` in this repository](/assets/draft) or [`assets/event` in the Mido's House repository](https://github.com/midoshouse/midos.house/tree/main/assets/event) for examples. Cannot be combined with `--preset`, `--settings`, or `--rsl`.
